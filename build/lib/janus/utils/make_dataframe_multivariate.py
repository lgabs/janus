...
def create_per_user_dataframe_multivariate(
    ...
    assert set(df_summary_daily.columns) == set(
        ALLOWED_COLS
    ), f"Use allowed cols plus 'conversion_value_cols': {ALLOWED_COLS}"
    ...
    # basic variables
    variants = list(df_summary_daily.variant_id.unique())
    ...
    for period in periods:
        for variant in variants:
            _df = df_summary_daily[
                ...
                & (df_summary_daily.variant_id == variant)
            ]
            exposures = _df.exposures.values[0]
            conversions = _df.conversions.values[0]
            not_converted = exposures - conversions

            variant_ids_converted = np.repeat(variant, conversions)
            variant_ids_not_converted = np.repeat(variant, not_converted)
            ...
            # Generate dataframes
            data_converted = {
                "variant_id": variant_ids_converted,
            ...
            data_not_converted = {
                "variant_id": variant_ids_not_converted,
